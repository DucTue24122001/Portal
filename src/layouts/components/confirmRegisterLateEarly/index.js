...
const ConfirmLateEarly = ({
  ...
  useEffect(() => {
    if ((successRegisterLateEarly || successUpdateLateEarly || successConfirmLateEarly) === true) {
      dispatch(LateEarlyActions.getRequest(manager_id))
    }
  ...
  const handleAdminRejectLateEarly = () => {
    ...
    dispatch(LateEarlyActions.rejectLateEarlyAdmin(dataReject, dataReject.admin_id))
  }
      ...
      >
        ...
        {nameStatus && isAdmin && (
          <Row className={style.status}>
            ...
            <Col xs={10} sm={5}>
              <span> Confirm | {moment(created_at).format('YYYY-MM-DD')}</span>
            </Col>
          </Row>
        )}
